...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/api/showcase":
            try:
                self._send_json(build_showcase())
            except Exception as e:
                self._send_json({"error": str(e)}, status=500)
            return
...
def main():
    ...
    print(f"  fleet APIs: /api/discovery /api/llm/* /api/agentcore/* /api/router/* /api/mesh /api/fleet/health /api/showcase", file=sys.stderr)
